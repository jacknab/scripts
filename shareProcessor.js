...
module.exports = function(logger, poolConfig) {
    ...
    connection.info(function(error, response) {
        ...
        if (!version) {
            logger.error(
                ...
                logComponent,
                logSubCat,
                'Could not detect redis version - might be super old or broken'
            );
        } else if (version < 2.6) {
            logger.error(
                logSystem,
    ...
    this.handleShare = function(isValidShare, isValidBlock, shareData) {
        ...
        if (isValidBlock) {
            // Custom logic for true solo mining. This file will break pool mining only use if your wanting a true solo mining setup
            redisCommands.push([
                'hincrbyfloat',
